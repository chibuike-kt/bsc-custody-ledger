...
final class ScanUsdtDepositsCommand extends Command
{
  ...
  protected function execute(InputInterface $input, OutputInterface $output): int
  {
    ...
    foreach ($logs as $log) {
      ...
      $amountRaw = (string)hexdec($amountHex); // NOTE: for very large amounts, replace with BCMath/gmp later

      try {
        $stmt = $pdo->prepare("
          INSERT INTO chain_deposits
            (id, chain, token_contract, tx_hash, log_index, from_address, to_address, amount_raw, block_number, status)
          VALUES
            (?, 'bsc', ?, ?, ?, ?, ?, ?, ?, 'detected')
        ");
        $stmt->execute([
